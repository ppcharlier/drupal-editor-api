...
| field access & validation | a write only touches the fields it sends. A field the account may not **edit** (`FieldItemList::access('edit')`, as Field Permissions and friends set it) is refused with `403` before anything is written; a field it may not edit is likewise never held against it at validation time (core's `filterByFieldAccess`). Constraints are checked on the fields the request **writes**, not on the whole entity — so a field that became required after the content was created no longer blocks an unrelated edit, a rename, or a revision restore |
| `cms` in `/config` | `"drupal"` — which server answers, so an app serving several CMS can tell which one an account belongs to. A stable identifier, never a display label: the client picks the name it shows. Optional and additive; older servers omit it |
