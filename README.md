...
| asset container / asset | media type with an image or file source / media item; `path` = `{mid}/{basename}`, no folders (`folders: []`, `can.move: false`); upload validated by the source field (extensions, size) and, for images, decodability (`FileIsImage`); `data` = `{ alt, title }` for images, `{ description }` for files; `embed: { entity_type: "file", uuid }` from the source file, omitted when the media item has none — copied by the app onto inserted `<img data-entity-*>`; `uuid` = the media item's own UUID and `thumbnail` = its thumbnail URL (core's `thumbnail` image style when present) are served on every asset summary |
